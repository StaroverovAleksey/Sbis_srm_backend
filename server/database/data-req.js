...
const setupCollection = async () => {
  ...
  const collection = dBase.collection(`data`);
...
class LicensesStore {
  ...
  async deleteField(id, data) {
    return (await this.collection).findOneAndUpdate({_id: objectId(id)}, {$unset: data});
  }

  async deleteLicense(id) {
    return (await this.collection).updateMany({licenses: id}, {$pull: {licenses: id}}, {multi: true});
  }
